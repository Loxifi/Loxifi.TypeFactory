...
namespace Loxifi
{
    ...
    public class TypeFactory
    {
        ...
        public TypeFactory(TypeFactorySettings settings)
        {
            ...
            foreach (string searchPath in searchPaths)
            {
                ...
                foreach (string loadPath in referencedPaths)
                {
                    ...
                    {
                        //Check for blacklist
                        string matchingLine = _settings.Blacklist.FirstOrDefault(b => Regex.IsMatch(Path.GetFileName(loadPath), b));
                        ...
                        {
                            FailedAssemblyLoad?.Invoke(this, new AssemblyLoadFailure(loadPath, ex));
                        }
                    }
        ...
        public T? GetAttribute<T>(MemberInfo p) where T : Attribute => GetCustomAttributes(p).First(a => a.Instance.GetType() == typeof(T)).Instance as T;
        ...
        public bool HasAttribute<T>(MemberInfo p) where T : Attribute => GetCustomAttributes(p).Any(a => a.Instance.GetType() == typeof(T));
        ...
        /// <returns>Does the member declare this attribute?</returns>
        public bool HasAttribute(MemberInfo p, Type t) => GetCustomAttributes(p).Any(a => a.Instance.GetType() == t);
        ...
        internal AttributeInstance[] GetCustomAttributes(MemberInfo p)
        {
            if (p is null)
            {
